...
resource "aws_api_gateway_integration" "get_public_url_api_integration" {
  ...
  type = "AWS"
  uri = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${aws_lambda_function.get_public_url_lambda.arn}/invocations"
...
resource "aws_lambda_permission" "get_public_url_api_lambda_permission" {
  ...
  source_arn = "arn:aws:execute-api:${var.region}:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.get_public_url_api.id}/*/*/*"
}
